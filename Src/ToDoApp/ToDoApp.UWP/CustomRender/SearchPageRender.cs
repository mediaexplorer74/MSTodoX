...
using ToDoApp.UWP.CustomRender;
...
namespace ToDoApp.UWP.CustomRender
{
    public class SearchPageRender : PageRenderer
    {
        ...
        protected /*override*/ void OnAttachedToWindow()
        {
            ...
            {
                //Workaround to re-add the SearchView when navigating back
                //to an ISearchPage, because Xamarin.Forms automatically removes it
                navigationPage.Popped += HandleNavigationPagePopped;
                navigationPage.PoppedToRoot += HandleNavigationPagePopped;
            }
        }

        //Adding the SearchBar in OnSizeChanged ensures the SearchBar
        //is re-added after the device is rotated, because Xamarin.Forms automatically removes it
        protected /*override*/ void OnSizeChanged(int w, int h, int oldw, int oldh)
        {
            ...
            if (Element is ISearchPage && Element is Page page 
                && page.Parent is NavigationPage navigationPage && navigationPage.CurrentPage 
                is ISearchPage)
            {
        ...
        //Workaround to re-add the SearchView when navigating back to an ISearchPage,
        //because Xamarin.Forms automatically removes it
        ...
        /*
        private void SearchView_QueryTextChange(object sender, 
        Android.Support.V7.Widget.SearchView.QueryTextChangeEventArgs e)
        {
        ...
        {
            return default;//(CrossCurrentActivity.Current?.Activity 
               as MainActivity)?.FindViewById<Android.Support.V7.Widget.Toolbar>(Resource.Id.toolbar);
        }
        */
    }
}